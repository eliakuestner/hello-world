...
const DEFAULT_USERS = [
  ...
  },
  {
    ...
    rolle: 'tekendienst',
    password: process.env.SERVICE_PASSWORD || 'service123',
  },
];
  ...
  if (!fs.existsSync(DATA_FILE)) {
    ...
    const defaultData = {
      users: DEFAULT_USERS.map((u) => ({
        ...
        e_mail: u.e_mail,
        rolle: u.rolle,
        passwort_hash: hashPassword(u.password),
        erstellt_am: now,
      })),
...
function ensureDefaultUsers(db) {
  ...
  DEFAULT_USERS.forEach((seed) => {
    const existing = db.users.find(
      (u) => u.id === seed.id || (u.e_mail || '').toLowerCase() === seed.e_mail.toLowerCase()
    );
    if (!existing) {
      db.users.push({
        id: seed.id,
        ...
        e_mail: seed.e_mail,
        rolle: seed.rolle,
        passwort_hash: hashPassword(seed.password),
        erstellt_am: now,
      });
      changed = true;
    } else {
      if (!existing.passwort_hash) {
        existing.passwort_hash = hashPassword(seed.password);
        changed = true;
      }
      if (!existing.rolle) {
        existing.rolle = seed.rolle;
        changed = true;
      }
    }
  });
...
function readDb() {
  ...
  const changed = ensureDefaultUsers(db);
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && urlObj.pathname === '/api/auth/login') {
    ...
    const email = (body.email || body.e_mail || '').trim().toLowerCase();
    const password = body.password || body.passwort || '';
    const user = db.users.find((u) => (u.e_mail || '').toLowerCase() === email);
    const defaultSeed = DEFAULT_USERS.find((u) => u.e_mail.toLowerCase() === email);
    if (!user) {
      send(res, 401, { message: 'Login fehlgeschlagen' });
      return;
    }
    const isValid = user.passwort_hash && verifyPassword(password, user.passwort_hash);
    if (!isValid) {
      if (defaultSeed && password === defaultSeed.password) {
        user.passwort_hash = hashPassword(password);
        writeDb(db);
      } else {
        send(res, 401, { message: 'Login fehlgeschlagen' });
        return;
    ...
    if (!user.rolle) user.rolle = defaultSeed?.rolle || 'tekendienst';
